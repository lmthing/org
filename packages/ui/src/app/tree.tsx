  ...
  return (
    <div>
      ...
      >
        ...
        <span className="truncate text-lm-text" title={node.label}>{node.label}</span>
        <KindBadge kind={node.kind} />
        {dur && <span className="text-lm-muted text-[10px] font-mono ml-auto shrink-0">{dur}</span>}
        {retries > 0 && <span className="text-lm-amber text-[10px] font-mono shrink-0" title={`${retries} retries`}>×{retries}</span>}
      </div>
...
export function ExecutionTree(): React.ReactElement {
  ...
  return (
    <nav aria-label="execution tree" className="h-full overflow-y-auto py-1">
      <div className="px-2 py-1 text-[10px] uppercase tracking-wider text-lm-muted flex items-center justify-between">
        <span>Execution</span>
        {queue && <span className="font-mono">q {queue.active}/{queue.max}</span>}
      </div>
      {rootId ? <TreeRowById id={rootId} depth={0} now={now} /> : (
        <div className="px-3 py-4 text-lm-muted text-[12px]">No activity yet. Send a message to start.</div>
      )}
    </nav>
  );
}
