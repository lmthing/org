 ...
 * prices/azure.json to the package root.
 *
 * Run: pnpm fetch-azure-prices
 *
...
const MODEL_MAP: Record<string, { product: string; inputSku: string; outputSku: string }> = {
  ...
  // SKU names verified via: pnpm fetch-azure-prices --dump
  'gpt-5.5': {
    product: 'Azure OpenAI GPT5',
    inputSku: 'GPT 5.5 Chat Inpt Glbl',
    outputSku: 'GPT 5.5 Chat outpt Glbl',
  },
};
...
function findPrice(items: PriceItem[], product: string, sku: string): number | null {
  const p = product.toLowerCase();
  const s = sku.toLowerCase();
  const item = items.find(
    (i) => i.productName.toLowerCase() === p && i.skuName.toLowerCase() === s,
  );
...
async function main(): Promise<void> {
  ...
  const outPath = join(dirname(fileURLToPath(import.meta.url)), '../prices/azure.json');
