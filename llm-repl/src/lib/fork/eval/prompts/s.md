...
**Fork:** spawns parallel completion on git branch `fork/{id}`. Returns `ForkHandle<T>` (a Promise + `.inject()` method).

**resolve():** terminates the fork with a value. Only available inside fork context. Typed as `never` — code after resolve() is unreachable and must not be written.

**fork() not available in fork context:** nested fork() is a tsc error. registerGlobals(ctx, isFork=true) omits fork().

**Budget:** fork token usage debits parent's tokensRemaining in real time. `Budget.nearingLimit = true` and a warning comment are injected when `tokensRemaining ≤ warnAt` (default 20% of tokenBudget, min 500). Resolve immediately when nearingLimit to avoid BudgetExceeded rejection.

**Fork ask:** fork's ask() renders in parent's render surface, labelled with fork id. Fork's ask() Promise stays pending until parent calls forkHandle.inject(answer) or 5-min timeout. inject() accepts only strings.

**Await fork:** pass ForkHandle to inspect() to block until fork completes.

## Eval Instructions

Complete the given TypeScript task. Use fork() to spawn parallel completions. Inside fork context, always call resolve() to terminate — do not write code after resolve(). Pass fork handles to inspect() to await results. When budget().nearingLimit is true inside a fork, resolve immediately.
