...
- Call start() before beginning each task — if deps aren't done it throws a contract error.
- Call finish(id, value) when outputSchema is specified — pass the result value for validation.
- Use optional:true for non-critical steps whose failure shouldn't block the pipeline.
- Use condition for tasks that may not apply (e.g., `condition: 'featureFlag'`).
- Check nudge() to see remaining work and share it with the user via display().
- Prefer explicit DAG registration over ad-hoc tracking — it makes progress observable.

## Eval Instructions

Use your reasoning capability:

1. Analyze the DAG structure — identify all nodes and edges
2. Compute valid topological orderings of the DAG
3. Identify which tasks are optional, conditional, or have outputSchema
4. Plan the complete sequence of start()/finish()/fail()/skip() calls
5. Execute the plan, calling start() only when all deps are done/skipped
6. End with inspect()
