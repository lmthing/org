...
1. Identify what values are relevant at this point in the session
2. Determine whether any async operations are in flight (Promise args to inspect)
3. Select an appropriate timeout if the operation may be slow
4. End the completion with inspect() passing all relevant values

The primary test: no code appears after inspect(). inspect() must be the final statement.
