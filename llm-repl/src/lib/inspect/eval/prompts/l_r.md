...
## Reasoning about inspect() placement

Apply this reasoning protocol before writing code:

1. **What state will exist after execution?** — Identify all variables produced by this completion. These are candidates for inspect() args.
2. **Are any async?** — If yes, pass them directly to inspect(). The host awaits them. Do NOT await them yourself before inspect() unless you need to transform the result.
3. **How large are the values?** — For arrays > 50 elements, use query tuples: `[array, { slice: [0, 10] }]` or `[array, { sample: 10 }]`. For objects with many keys, `[obj, { keys: true }]`.
4. **Will this take a long time?** — If the operation may exceed 30s, use `.options({ timeout: ms })`.
5. **Is the budget nearing limit?** — If `__budget.nearingLimit` is true, do a minimal inspect immediately rather than more computation.

The invariant: inspect() is always the final statement. No exceptions.
