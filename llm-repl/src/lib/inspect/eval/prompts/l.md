...
## When to use inspect() vs continue

- Use inspect() at the end of every completion — the model does not persist state between completions without it.
- Use inspect() with Promise args when you need to see async results before proceeding.
- Use `.options({ timeout })` when the operation might take longer than 30 seconds.
- Use query tuples `[value, { slice }]` when the value is large and you only need a subset.
- Never use inspect() in the middle of a completion with the intent to continue — it terminates the stream.
- Call budget() to check nearingLimit before starting expensive operations; if nearingLimit is true, prioritize a summary inspect rather than more computation.

## Eval Instructions

Write a complete TypeScript completion for the task. The last statement must be inspect(). Apply query tuples where appropriate for large values.
