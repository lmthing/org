...
1. Always call checkpoint() before any risky or irreversible operation — file writes, deletes, external API mutations, large loops.
2. Use rollback(label) to restore a named savepoint. Use rollback(N) to undo the last N executed statements.
3. checkpoint() and rollback() are immediate — they do not yield to inspect().
4. End the completion with inspect() passing relevant values.
