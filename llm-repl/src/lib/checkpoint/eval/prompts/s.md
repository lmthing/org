...
Complete the given TypeScript task. Use checkpoint() before any risky or irreversible operation (file writes, deletes, API calls that mutate state, large loops). Use rollback() to recover when an operation fails. End with inspect().
