...
- **Project-app globals resolve against `projectRoot`, never `LMTHING_SPACE_DIR`** — `db`, `writePage`/`writeApi`/`writeHook`, and `createProject`/`selectProject` are project-rooted (a session with no `projectRoot` gets none of them injected); the agent's `db.*` is a **synchronous** host call while `api/`/`hooks/` Node handlers get the same methods as `AsyncDbApi` (`Promise`-returning, cross-thread proxy — every write still lands in the main process). → `project-as-application.md` · `SPACE_DEVELOPMENT.md` §7
- **`api/` runs Node-worker-isolated** (crash boundary, not a security boundary — the pod is), **`pages/` are real client-side React** (no pod-side loader), **`hooks/` `database` dispatch is in-proc and decoupled from the write** (enqueue → drain after the eval, never re-entrant); a project-app agent's chat history persists under `<project>/spaces/<spaceId>/sessions/`, **not** `<project>/sessions/`. → `project-as-application.md`
