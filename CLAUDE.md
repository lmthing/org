...
- **`api/` runs Node-worker-isolated** (crash boundary, not a security boundary — the pod is), **`pages/` are real client-side React** (no pod-side loader); a project-app agent's chat history persists under `<project>/spaces/<spaceId>/sessions/`, **not** `<project>/sessions/`. → `project-as-application.md`
