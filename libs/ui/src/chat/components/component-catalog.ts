 ...
 * cross-platform UI vocabulary that a `display()`/`ask()` descriptor may name.
 *
 * VENDORED from `@lmthing/core`'s `libs/core/src/ui/catalog.ts` (the module
 * that used to be imported as `@lmthing/core/ui`) when `@lmthing/ui` severed
 * its dependency on `@lmthing/core`. Copied verbatim (`CatalogProp`,
 * `CatalogEntry`, `DISPLAY_CATALOG`, `FORM_CATALOG`, `CATALOG`,
 * `CATALOG_BY_NAME`) — only `catalogSummary()`/`catalogDts()`/
 * `isFormComponent()`/`CATALOG_NAMES` were dropped, since those exist solely
 * to generate the model-facing system prompt / DTS overlay inside the old
 * QuickJS runtime and have no reader here. If core's catalog ever changes,
 * this copy needs the same edit — there is no longer a single source of truth
 * shared between the two packages.
 *
 * Each entry names a descriptor `type`, documents its props, and marks whether
 * it is a `display` (output) or `form` (interactive) component. Type names are
 * matched case-insensitively by the renderers, so a descriptor may spell
 * `Stack` or `stack`.
 */

export interface CatalogProp {
  name: string;
  /** TypeScript type literal used verbatim in the generated DTS (core only; unused here). */
  type: string;
  optional?: boolean;
  doc?: string;
}
