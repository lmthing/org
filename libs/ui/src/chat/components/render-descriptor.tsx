...
export function renderDescriptor(d: unknown, key?: React.Key): React.ReactNode {
  ...
  if (!isDescriptor(d)) return <Prim.Text className="text-lm-muted" fontFamily="$mono">{preview(d, 400)}</Prim.Text>;
  ...
  switch (d.type.toLowerCase()) {
    ...
    case 'h2': return <Prim.Text as="h2" key={key} className="text-lm-text" fontSize="$base" fontWeight="$semibold" marginVertical="0.25rem">{body}</Prim.Text>;
    case 'h3': return <Prim.Text as="h3" key={key} className="text-lm-text" fontSize="$sm" fontWeight="$semibold" marginVertical="0.25rem">{body}</Prim.Text>;
    ...
    case 'kbd': return <Prim.Text as="kbd" key={key} className="border-lm-border bg-lm-panel" fontFamily="$mono" color="11px" borderWidth={1} borderRadius="$radius" paddingHorizontal="$1">{body}</Prim.Text>;
    case 'code': return <Prim.Text as="code" key={key} className="text-lm-cyan bg-lm-bg" fontFamily="$mono" paddingHorizontal="$1" borderRadius="$radius">{body}</Prim.Text>;
    ...
    case 'quote': return <Prim.Box as="blockquote" key={key} className="border-lm-border text-lm-muted" borderLeftWidth={2} paddingLeft="$2" fontStyle="italic" marginVertical="0.25rem">{body}</Prim.Box>;
    case 'link': return <Prim.Link key={key} href={String(props['href'] ?? '#')} target="_blank" rel="noreferrer" className="text-lm-accent" textDecorationLine="underline">{body}</Prim.Link>;
    ...
    case 'divider': return (
      <Prim.Row key={key} className="text-lm-muted" gap="$2" marginVertical="$2" color="11px" alignItems="center">
        <Prim.Text className="border-lm-border" flexGrow={1} flexShrink={1} flexBasis="0%" borderTopWidth={1} />{props['label'] ? <Prim.Text>{String(props['label'])}</Prim.Text> : null}<Prim.Text className="border-lm-border" flexGrow={1} flexShrink={1} flexBasis="0%" borderTopWidth={1} />
      </Prim.Row>
    );

    // ── surfaces ──
    case 'card': case 'panel': return (
      <Prim.Box key={key} className="border-lm-border bg-lm-panel2" borderWidth={1} borderRadius="$radius" padding="$2" marginVertical="0.25rem">
        {props['title'] ? <Prim.Box className="text-lm-text" fontSize="$sm" fontWeight="$semibold" marginBottom="0.25rem">{String(props['title'])}</Prim.Box> : null}{body}
      </Prim.Box>
    );
    ...
    case 'keyvalue': {
      ...
      return <Prim.Box as="dl" key={key} color="12px" marginVertical="0.25rem">{Object.entries(pairs).map(([k, v]) => <Prim.Row key={k} gap="$2"><Prim.Text as="dt" className="text-lm-muted" minWidth="120px">{k}</Prim.Text><Prim.Text as="dd" className="text-lm-text">{String(v)}</Prim.Text></Prim.Row>)}</Prim.Box>;
    }
    case 'timeline': {
      const items = (props['items'] as { title: string; time?: string; detail?: string }[]) ?? [];
      return <Prim.List key={key} className="border-lm-border" borderLeftWidth={1} paddingLeft="$3" marginVertical="0.25rem">{items.map((it, i) => <Prim.ListItem key={i} marginBottom="0.25rem"><Prim.Box className="text-lm-text">{it.title}{it.time ? <Prim.Text className="text-lm-muted" color="10px" marginLeft="0.5rem">{it.time}</Prim.Text> : null}</Prim.Box>{it.detail ? <Prim.Box className="text-lm-muted" color="11px">{it.detail}</Prim.Box> : null}</Prim.ListItem>)}</Prim.List>;
    }
    ...
    case 'progressbar': {
      ...
      return <Prim.Box key={key} marginVertical="0.25rem"><Prim.Box className="bg-lm-panel" height="$2" borderRadius="$radius" overflow="hidden"><Prim.Box className="bg-lm-accent" height="100%" style={{ width: `${pct}%` }} /></Prim.Box>{props['label'] ? <Prim.Box className="text-lm-muted" color="10px" marginTop="0.125rem">{String(props['label'])}</Prim.Box> : null}</Prim.Box>;
    }
