/**
 * Cross-platform form normalization — flattens an `ask()` form descriptor into
 * a flat list of `FieldSpec`s, then collects + coerces values.
 *
 * VENDORED (runtime-ported) from `@lmthing/core`'s `libs/core/src/ui/form.ts`
 * (previously imported as `@lmthing/core/ui`'s `isFormDescriptor`/
 * `flattenForm`/`coerceValue`/`defaultFor`/`FieldSpec`) when `@lmthing/ui`
 * severed its dependency on `@lmthing/core`. Copied byte-for-byte, including
 * `KIND_BY_TYPE` and the internal `collect`/`fieldFromDescriptor` walk — this
 * module is self-contained (unlike `descriptor-protocol.ts`, it does NOT
 * depend on the component catalog: `isFormDescriptor`/`KIND_BY_TYPE` name
 * their own fixed set of form-control type strings).
 *
 * `isCatalogForm` (core's helper that cross-checks a type name against the
 * catalog's `kind: 'form'` entries) was NOT ported: nothing under
 * `libs/ui/src` imports it — `CatalogForm.tsx`/`AskBlock.tsx`/`Message.tsx`
 * only ever needed the five exports above. If `@lmthing/ui` later needs it,
 * port it from the same source file, importing `CATALOG_BY_NAME` from
 * `../component-catalog`.
 *
 * Both the web (`CatalogForm`) and terminal (`InkForm`, core-side, not ported
 * here) renderers used to flatten an `ask()` form descriptor into the same flat
 * list of `FieldSpec`s with the same semantics, then collect + coerce values
 * the same way — so `ask(<Form>…</Form>)` behaved identically on both
 * surfaces. This copy keeps the web side of that promise; the terminal side is
 * core's problem now, not `@lmthing/ui`'s.
 */
