...
import { GITHUB_LOGIN_AUTHENTICATED, GITHUB_LOGIN_DEVICE_CODE, GITHUB_LOGIN_DEVICE_CODE_HINT, GITHUB_LOGIN_DEVICE_CODE_WAITING, GITHUB_LOGIN_ICON } from '../props.js'
...
export function GithubLogin() {
  ...
  if (isAuthenticated && user) {
    return (
      <Stack row gap="sm" {...GITHUB_LOGIN_AUTHENTICATED}>
        <Avatar size="sm">
      ...
      {deviceCodePrompt ? (
        <Card>
          <CardBody>
            ...
            <Prim.Link href={deviceCodePrompt.verificationUri} target="_blank" rel="noopener noreferrer">
              <Label>{deviceCodePrompt.verificationUri}</Label>
            </Prim.Link>
            <Caption {...GITHUB_LOGIN_DEVICE_CODE_HINT}>And enter code:</Caption>
            <Code block {...GITHUB_LOGIN_DEVICE_CODE}>
              {deviceCodePrompt.userCode}
            </Code>
            <Caption muted {...GITHUB_LOGIN_DEVICE_CODE_WAITING}>Waiting for authorization...</Caption>
          </CardBody>
        </Card>
      ) : (
        <Button onClick={() => login().catch(console.error)} variant="primary">
          <Prim.Svg {...GITHUB_LOGIN_ICON} viewBox="0 0 24 24" fill="currentColor">
