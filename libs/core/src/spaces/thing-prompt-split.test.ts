...
/**
 * THING's `instruct.md` was one 1270-line file: every routing decision AND the full detail of
 * every route, on every turn, whether or not the turn went near that route — including the whole
 * team surface, which a personal workspace can never use. It is now split —
...
describe('user-thing/thing — the instruct/knowledge split holds together', () => {
  ...
  it('keeps the always-on body under the ratchet', () => {
    ...
    expect(
      lines,
      `thing/instruct.md is ${lines} lines — it was 1270 before the split. Move detail into a playbooks aspect rather than raising this.`,
   ...
   * still be safe. These four are therefore ALWAYS ON — asserted on `instruct.md` alone, not on the
   * corpus — and the moment one of them drifts into `conduct.md` it stops holding on the turns that
   * skip the load. Everything ELSE about teams (the directory, the readers, the writers, the
   * ten-point conduct) is correctly behind the load: it is needed only once a request reaches past
