 ...
 * host resolves the request. The host resolver runs IN THE POD and makes the
 * outbound REST call itself — there is no gateway round-trip. Injected only when
 ...
 *
 * The provider token never enters the sandbox: the sandbox supplies only
 * `provider` + `{ method, path, query?, body?, headers? }`; the pod-side resolver
 * (`@lmthing/cli`'s `server/connections.ts`) reads the token from the pod env
 * (bring-your-own-token), attaches it, and pins the outbound host to the
 * provider's API base.
 */
