/** Read a text file from your scratch sandbox (call createScratch() first). `content` has
 *  1-based line numbers (useful for referencing lines in editFile); `raw` has the unmodified
 *  text (use this for JSON.parse and structured data). Paths resolve inside the scratch dir. */
export function readFile(
  path: string,
  opts?: { offset?: number; limit?: number },
): { ok: boolean; content: string; raw: string; lines: number; truncated: boolean; error?: string } {
  const r = scratchReadRaw(path, opts);
