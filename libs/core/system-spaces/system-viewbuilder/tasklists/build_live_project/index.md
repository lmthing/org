...
the types it must satisfy exist. Same entry point and same input as `system-appbuilder`'s
`build_live_project`, so the two builders are interchangeable at one call site; **the difference is
the medium of the UI.**
...
The second is that the whole failure class the appbuilder defends against — a dangling import, a
re-cased field, a stray comma in a JSX container, a surface token used as a text colour — cannot be
authored here, so none of that machinery exists in this pipeline. What CAN go wrong is different, and
...
to fix: the appbuilder's checks (every table ref real, no duplicate name/route, every `[id]` route has
