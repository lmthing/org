...
// Views actually on disk (ground truth): every persisted page spec. `_shell.view.json` is the app
// shell, not a route, and `components/` is a DIRECTORY — neither is a page.
const diskPages = (listProjectDir('pages').entries || [])
