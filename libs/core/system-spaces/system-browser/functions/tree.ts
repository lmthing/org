...
/**
 * Simplified semantic DOM tree (role, name, value, backendNodeId per node). The default starting point for an unfamiliar page. Pass `backendNodeId` to scope, `maxDepth` to limit depth, `url` to navigate and read in one call.
 */
export async function tree(args?: { url?: string; backendNodeId?: number; maxDepth?: number; timeout?: number }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  const endpoint = (process.env['LIGHTPANDA_MCP_URL'] ?? 'http://127.0.0.1:9223').replace(/\/+$/, '');
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'tree', arguments: args ?? {} } };
