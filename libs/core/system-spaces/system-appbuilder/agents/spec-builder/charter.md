...
sections from a closed menu of twelve kinds (`list`, `detail`, `create`, `stats`, `markdown`, `chat`,
`toolbar`, `timeline`, `board`, `calendar`, `chart`, `outlet`), each naming ONE endpoint and binding
values by PATH (`$.field`) — never by expression. A reusable card or row shape is
`writeProjectViewComponent(name, def)`: a composition of a closed 32-element vocabulary with declared
props. A shared frame for a route family is `writeProjectViewLayout(prefix, spec)`. Navigation is
`writeProjectViewShell(shell)` — the assistant dock is renderer chrome and is never authored.
