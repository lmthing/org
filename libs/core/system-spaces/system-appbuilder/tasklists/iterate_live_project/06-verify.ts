/**
 * The verify gate for an ITERATION — HOST-RUN, computes everything FRESH from the live project's
 * own disk/build state. A close relative of `build_live_project/16-verify.ts` and
 * `repair_live_project/01-diagnose.ts` (same three ground truths: `buildProjectApp()`,
 * `validateAppViews()`, `renderSmokeViews()`, plus the "references a table that does not exist"
 * scan) — duplicated rather than imported because a code node is transpiled standalone and
 * evaluated in a worker, so a relative import would not resolve at require time.
 *
 * A change to ONE part of the app can break another (a renamed column an unrelated endpoint still
 * queries, two views that now share a route) — so this checks the WHOLE app, not just what
 * `plan_change` touched. As in `repair_live_project`, a finding on an artifact that was never
 * written at all (`toAuthor`) is kept apart from an existing-but-wrong artifact (`offending`),
 * because `fix`'s repair prompt assumes real content to edit.
 */

export const node = {
  id: 'verify',
  dependsOn: ['implement_tables', 'implement_endpoints', 'implement_components', 'implement_views'],
  output: {
    ok: 'boolean',
    offending: 'array',
...
export async function run(ctx: Ctx, _inputs: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  // table that does not exist — builds clean, 500s at runtime. High-value here specifically
  // because an iteration often ADDS a column/table and a sibling endpoint can drift out of sync.
