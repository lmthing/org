 ...
 * (`05-plan_endpoints.md`), and `implement_views` is told to read exactly those keys — but
...
export const node = {
  ...
  dependsOn: ['validate_contract', 'plan_tables', 'plan_endpoints', 'plan_view_components'],
  output: {
    ok: 'boolean',
    written: 'boolean',
    path: 'string',
...
export function readContract(inputs: Record<string, unknown>): Contract {
  ...
  return {
    ...
    components: pick<ContractComponent>('components', 'plan_view_components'),
  };
}
