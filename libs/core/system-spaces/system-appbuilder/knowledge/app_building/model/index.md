...
- **`database/<table>.json`** — the data model: JSON table schemas. `writeProjectTable(name, schema)`.
  Every table, column, and relation carries a required `description`. **Prefer**
...
- **`api/<name>.query.json`** — every HTTP endpoint is authored with
  `writeProjectQuery(name, query)`. Its declarative list/get/aggregate/create/update/toggle/delete
  IR generates `api/<path>/<METHOD>.ts` from the same data, so handler and contract cannot disagree.
  Parent/child reads use declared table relations with `include` and row `compute`; a requirement
  outside the current IR extends that mechanism rather than adding a hand-written handler.
