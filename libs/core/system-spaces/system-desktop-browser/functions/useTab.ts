...
/**
 * Switch to an existing tab by the target id from listTabs. Everything else you do afterwards applies to that tab, and the person's pane follows.
 */
export async function useTab(args: { targetId: string }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'useTab', arguments: args } };
