...
/**
 * The visible text of the current page, as a person reading the screen would see it. Free of script, style and markup. This is the right tool for understanding a page; use readHtml only when you need the markup itself.
 */
export async function readText(): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'readText', arguments: {} } };
