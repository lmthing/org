...
/**
 * Type text using real key events, so pages that react to each keystroke (search suggestions, live validation) behave normally. Give a selector or index to click into that field first; existing content is replaced unless `clear` is false. Pass `submit: true` to press Enter afterwards.
 */
export async function typeText(args: { text: string; selector?: string; index?: number; clear?: boolean; submit?: boolean }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'typeText', arguments: args } };
