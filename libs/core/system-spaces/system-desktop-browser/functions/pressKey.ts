...
/**
 * Press a single key by its DOM name — Enter, Tab, Escape, ArrowDown, Backspace. For typing text, use typeText instead.
 */
export async function pressKey(args: { key: string }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'pressKey', arguments: args } };
