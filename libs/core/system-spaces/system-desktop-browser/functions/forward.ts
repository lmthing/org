...
/**
 * Go forward one entry in the browser's history. Reports an error if there is nowhere to go forward to.
 */
export async function forward(): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'forward', arguments: {} } };
