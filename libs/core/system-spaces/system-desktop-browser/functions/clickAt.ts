...
/**
 * Click an element by its index from `elements`, or by CSS selector. This is a REAL mouse click at the element's centre — the pointer moves there, hover states fire, and the person watching the pane sees it happen. Scrolls the element into view first, and reports whether the click navigated.
 */
export async function clickAt(args: { index?: number; selector?: string }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'clickAt', arguments: args } };
