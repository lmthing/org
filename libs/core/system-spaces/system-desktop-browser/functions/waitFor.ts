...
/**
 * Wait until a CSS selector appears, or — with no selector — until the document has finished loading. Returns an error if it never appears within `timeout` (default 10s), which is a real answer: the thing you expected is not there.
 */
export async function waitFor(args?: { selector?: string; timeout?: number }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'waitFor', arguments: args ?? {} } };
