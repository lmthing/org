...
/**
 * Go back one entry in the browser's history. Reports an error if there is nowhere to go back to, rather than silently doing nothing.
 */
export async function back(): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'back', arguments: {} } };
