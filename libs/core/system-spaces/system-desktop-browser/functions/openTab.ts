...
/**
 * Open a new tab at a URL and switch to it. Use this rather than `open` when you need to keep the current page — comparing two pages, or following a link without losing your place.
 */
export async function openTab(args: { url: string }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'openTab', arguments: args } };
