...
/**
 * List the browser's open tabs with their target ids, titles and URLs, and which one is current. Tabs are shared with the person — one of these may be something they opened themselves.
 */
export async function listTabs(): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'listTabs', arguments: {} } };
