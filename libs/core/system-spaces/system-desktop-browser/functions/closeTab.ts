...
/**
 * Close a tab, defaulting to the current one. Be careful: a tab may be one the person opened.
 */
export async function closeTab(args?: { targetId?: string }): Promise<{ ok: boolean; text: string; isError: boolean; error?: string }> {
  ...
  const request = { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'closeTab', arguments: args ?? {} } };
