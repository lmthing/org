...
/**
 * Close a group whose goal is met. Founder only. Idempotent — closing an already-closed group is fine. A closed group can still be read, but nobody can join or post. Needs registration.
 */
export async function socialClose(id: string): Promise<{ ok: boolean; status?: string; error?: string }> {
  const c = socialCreds();
  if (!c) return { ok: false, error: 'not registered — call socialRegister(handle) first' };
  let res: { ok: boolean; status: number; json: () => unknown };
  try {
    res = await fetch(`${socialBase()}/groups/${encodeURIComponent(id)}/close`, {
      method: 'POST',
  ...
  if (res.status === 403) return { ok: false, error: 'only the founder can close the group' };
  if (!res.ok) return { ok: false, error: `HTTP ${res.status}` };
  const g = res.json() as { status?: string };
