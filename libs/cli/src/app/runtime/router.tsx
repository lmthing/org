...
// The pure matcher (DOM-free, React-free) lives in the shared module so the
// per-project page bundle AND a future prebuilt app-shell import ONE matcher
// instead of forking it. Re-exported below so `@app/runtime` is unchanged.
import {
  matchRoutes as matchRoutesPure,
  stripPagesPrefix,
  type RouteMatch as RouteMatchBase,
} from '@lmthing/ui/view/router';
...
// ── Path matching ─────────────────────────────────────────────────────────────
//
// The matcher itself (`matchRoutes`, `stripPagesPrefix`, and the `:param`-segment
// parsing) is DOM-free and React-free, so it lives in the shared module
// `@lmthing/ui/view/router` and is re-exported here unchanged. `RouteEntry`
// satisfies that module's `RoutePattern` (`{ routePath: string }`) structurally,
// so the generic matcher returns this package's `RouteEntry` back out, component
// and all — see `libs/ui/src/view/router.ts` for the static-beats-param ranking
// and the stray `/pages/`-prefix tolerance (both load-bearing; do not lose them).

/**
 * A successful route match: the winning {@link RouteEntry} (component included)
 * plus the captured `:param` → value map.
 *
 * Narrowed from the shared module's generic `RouteMatch<T>` (in
 * `@lmthing/ui/view/router`) by substituting `T = RouteEntry`, so callers that
 * pass a `RouteEntry[]` table see exactly the type they did before the
 * extraction.
 */
export type RouteMatch = RouteMatchBase<RouteEntry>;

/** Re-exported from `@lmthing/ui/view/router` — see there for the rationale. */
export { stripPagesPrefix };

/**
 * Match a concrete client path against a route table (`:param` → capture).
 *
 * Thin re-export of the shared matcher in `@lmthing/ui/view/router` (where the
 * static-beats-param ranking and `/pages/`-prefix tolerance live), pinned to this
 * package's `RouteEntry` so the matched `entry` keeps its `Component`.
 */
export function matchRoutes(routes: RouteEntry[], clientPath: string): RouteMatch | null {
  return matchRoutesPure(routes, clientPath);
}
