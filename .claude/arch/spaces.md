...
- **The architect builds spaces one file at a time (no monolithic spec).** Earlier the architect handed a single giant `ScaffoldSpec` object to a `scaffoldSpace()` function; models malformed that object constantly (a ~300-line `normalizeSpec`/`validateSpecShape` layer existed only to defend against it). That is gone. The `synthesize_and_run` build task now writes each file with a small per-file builder in `system-spaces/system-architect/functions/`: `writeAgentFile` (instruct.md frontmatter+body), `writeTaskFile` (one `NN-<id>.md`, auto-ordinaled), `writeKnowledgeIndex`/`writeKnowledgeOption`, `writeFunctionFile` (typechecks the source on write via the `typecheckSource` host primitive), `writeComponentFile`. `validateSpace` is the gate before `registerSpace`. Each builder overwrites by path, so `iterate_space` reuses the same calls to mutate a space.
