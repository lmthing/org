...
if [ ! -f "$ENCRYPTED_FILE" ]; then
  echo "[session-start] .env.encrypted not found — skipping secret injection." \
    "Run scripts/encrypt-env.sh to create it."
  exit 0
fi

if [ -z "${ENV_DECRYPT_KEY:-}" ]; then
  echo "[session-start] ENV_DECRYPT_KEY is not set — cannot decrypt secrets." \
    "Add it in Claude Code web Settings → Environment variables."
  exit 0
fi
...
echo "[session-start] Injected $injected secret(s) from .env.encrypted."
