...
- `system-research` — Research agent `researcher` (`charter.md` + `instruct.md`, `defaultAction: research`). Ships **no functions of its own** — its tasks reach the web solely through `system-global`'s `webSearch`/`webFetch` (declared per-task via `functions:` frontmatter). Two tasklists: `research` (single goal task `answer` — one web search, a concise grounded answer + a few sources; the agent's default action) and `deep_research` (five tasks: `scope` (broad survey) → `plan` (decompose into sub-questions) → `investigate` (`forEach: plan.questions`, parallel per-question web research) → `synthesize` (cluster findings into themes) → `summarize` (goal task — structured executive-summary/findings/conclusion/sources report)). Always delegatable, e.g. `delegate('system-research', 'researcher', 'research', { query })` or with `'deep_research'` for the full report — the architect uses it for all web research.
- `user-thing` — **THE main user-facing orchestrator** (single agent, model-driven, no forced tasklist). Triage per request: answer directly, `delegate('system-research', …)` for research, `delegate('system-architect', 'architect', 'synthesize_and_run', …)` to build a specialist, `delegate('system-engineer', …)` to code, or `delegate('user-memory', …)` to save/recall user facts. Default agent in the `lmthing` project server. Reads per-project `instructions.md` + `documents/` (rooted at the project dir).
