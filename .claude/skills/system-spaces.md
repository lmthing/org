...
- `integration-google` / `integration-slack` / `integration-github` — user-OAuth **connection** spaces: one model-driven agent each (`google`/`slack`/`github`) that acts on the user's OWN connected account via typed wrappers over the `callConnection(provider, req)` global. They declare `capabilities: [ { connections:use: { providers: [<svc>] } } ]` (the only system agents besides `system-appbuilder` with `capabilities:`), which narrows `callConnection`'s `provider` to their service. Functions are pure request-builders/response-shapers (no npm SDKs); the gateway pins the API host + attaches the OAuth token, so the agent passes only a relative `path` and never sees credentials. No tasklists — model-driven, like THING. `delegate('integration-google', 'google', …)` etc.
