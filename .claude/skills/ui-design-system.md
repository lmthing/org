...
- Terminal → `libs/cli/src/render/ink-form.tsx` (interactive, sequential field stepping via `useInput`, wired into `InkRenderHost.ask` in human mode).

Both share flatten/coerce logic in `libs/core/src/ui/form.ts` (`flattenForm`/`coerceValue`/`defaultFor`/`isFormDescriptor`).

## Ink-compatibility layer for web

`libs/ui/src/chat/compat/` (`@lmthing/ui/chat`): web React mirrors of `ink`/`ink-text-input`/`ink-select-input` (Box, Text, Spacer, Newline, Static, Transform, useInput, TextInput, SelectInput, MultiSelect, ConfirmInput) that map Ink props to themed CSS. `serve.ts` aliases bare `ink*` imports here, so a single Ink-flavored component source runs in the browser unchanged.

## Theming

`libs/ui/src/theme/theme.ts` + `app/styles.css`: tokens defined per `[data-theme]` (dark default + light), exposed as both Tailwind `--color-lm-*` and plain `--lm-*` vars; runtime-switchable (DevTools header toggle, persisted). A space's optional `theme.json` is injected by `serve.ts` as `:root` overrides.
