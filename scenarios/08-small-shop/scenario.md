...
| **III — Agent-processed form** | a `POST` to `/app/ceramics-shop/api/<form>` returns ≥202; an **agent turn fires** (via `db.insert`→emitter→hook, not `ctx.spawn`); a sale row with a NEW token lands + stock decremented (before/after) | US-5 |
