...
| `judge.md` | the runner-judge-fixer: launch the runner, poll step evidence (2-min log checks), judge each step, fix the first failure at the lowest rung, verify with `--resume/--from`, drive to fully green. Takes the edit-lock; **signals the orchestrator to commit — never commits itself.** |
...
| `orchestrator.md` | the Opus orchestrator's own durable runbook: fan-out, concurrency, edit-lock, review+commit gate, the 5h-usage guard, the 400k-token handoff. |

## Who commits

The Opus **orchestrator** is the sole committer. Subagents leave changes uncommitted in the shared
`main` working tree under the edit-lock and signal the orchestrator, which reviews the diff (hard
anti-overfit + typecheck/test/lint gates) and commits+pushes to `main`, or feeds back for a revision.

## State (gitignored — `state/`)

- `state/orchestrator-state.json` — the orchestrator's live ledger.
- `state/edit.lock/` — the shared-tree source-edit lock (atomic `mkdir`).
- `state/<id>.handoff.md` — per-scenario resume note for the 400k-token handoff.
