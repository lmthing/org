...
**Fork:** spawns parallel completion on git branch. Returns Promise<ForkResult<T>>.

**resolve():** terminates the fork with a value. Only available inside fork context. Typed as `never` — no code after it.

**fork() not available in fork context:** nested fork() causes tsc error.

**Budget:** fork token usage debits parent. `Budget.nearingLimit = true` + warning at warnAt threshold.

**Fork ask:** fork's ask() renders in parent surface. Parent calls forkHandle.inject(answer) to respond.

**Await fork:** pass ForkHandle to inspect() to block until fork completes.

## Eval Instructions

Complete the given TypeScript task. Use fork() to spawn parallel completions. Inside fork context, always call resolve() to terminate. Pass fork handles to inspect() to await results.
