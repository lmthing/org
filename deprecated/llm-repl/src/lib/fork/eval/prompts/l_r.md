...
**Fork:** spawns parallel completion on git branch `fork/{id}`. Returns `ForkHandle<T>` which extends `Promise<ForkResult<T>>` and adds `.forkId: string` and `.inject(answer: string): void`. The fork branch is preserved for forensics.
...
**Seeded scope:** child fork starts with a copy of parent's heap.bin scope minus `exclude` vars. Session-space functions are re-injected as host-bridged globals in the child context.

**Budget:** fork token usage debits parent's tokensRemaining in real time. `tokenBudget` is capped internally at `min(tokenBudget, parent.tokensRemaining)` at spawn time. When `tokensRemaining ≤ warnAt` (default: max(20% of tokenBudget, 500)): `Budget.nearingLimit = true`, trace emits `fork_budget_warning`, warning comment injected into fork's next `__budget`. Fork is rejected with `BudgetExceeded` rejection if cap exhausted before resolve().

**Fork ask:** fork's ask() renders in parent's render surface, labelled with fork id. Fork's ask() Promise stays pending until parent calls `forkHandle.inject(answer: string)` or 5-min timeout. inject() only accepts strings — non-string is `kind: "contract"` error. inject() when no pending ask is silently ignored.

**Await fork:** pass ForkHandle directly to inspect() to block until fork completes before next reconstruct. The `__fork_asks` section appears in reconstruction while ask is pending.

**Trace events:** `fork_spawn`, `fork_resolve`, `fork_reject`, `fork_budget_warning`, `fork_ask`, `fork_ask_inject`, `fork_ask_timeout`.

## Eval Instructions

Complete the given TypeScript task. Follow these rules:

1. Use `fork({ instruction: '...' })` to spawn parallel completions. Returns ForkHandle<T> — a Promise + `.forkId` + `.inject()`.
2. Inside fork context: always call `resolve(value)` to terminate. Do not write any code after resolve() — it is typed `never`.
3. When `budget().nearingLimit` is true inside a fork, resolve immediately with partial results to avoid BudgetExceeded rejection.
...
6. Use `forkHandle.inject(answer)` to respond to a fork's pending ask(). Only strings are valid — non-string throws `kind: "contract"` error.
7. Do not use fork() inside a fork — it is omitted from fork context globals.
8. The `__fork_asks` section in reconstruction lists forks with pending asks — respond to these before proceeding.
9. `tokenBudget` caps fork's total token usage. `warnAt` (default: max(20% of cap, 500)) triggers nearingLimit before exhaustion.
