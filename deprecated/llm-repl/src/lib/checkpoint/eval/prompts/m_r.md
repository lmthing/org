...
1. Identify the risky operations in the task (mutations, deletes, external calls, large loops).
2. Determine the correct checkpoint label to use before each risky operation.
3. Determine if rollback is needed and by what target (label or count).
4. Write the completion following the use pattern. End with inspect().
