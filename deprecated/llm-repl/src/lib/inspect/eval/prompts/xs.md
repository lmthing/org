...
**inspect() aborts the stream:** calling inspect() commits the current state and signals the host to end this completion and start fresh context. Every completion must end with inspect().

**No dead code after inspect():** statements written after inspect() never execute. inspect() is always the last call in a completion.

**inspect() args:** pass variables you want expanded in the next context. Pass Promises directly — they'll be awaited.

**Soft timeout:** use `.options({ timeout: ms })` to set how long to wait for Promises passed to inspect().

**budget():** synchronous — returns current Budget without awaiting.

## Eval Instructions

Write a TypeScript completion for the given task. Always end with inspect(). No statements after inspect().
