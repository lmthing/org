...
**Deletion cascade:** removing `functions/MyClass.ts` via `remove()` marks all scope instances of MyClass as orphaned — they are nullified by the runtime on the next inspect.

**Pattern — capture function:**
```typescript
const src = `function formatDate(d: Date): string { return d.toISOString().split('T')[0]; }`;
Space.current().addFunction('formatDate', src);
inspect();
```
...
const current = await space.read('functions/calculate.ts') as string;
await space.patch('functions/calculate.ts', 'return x + 1', 'return x + 2');
inspect();
```

**Pattern — loadFunction expand:**
```typescript
const space = Space.current();
space.loadFunction('TaskManager', { expand: true });
inspect();  // full interface appears in next prompt
```

**Pattern — agent + knowledge:**
```typescript
const space = Space.current();
space.addAgent('analyzer', { model: 'claude-3', instruct: 'Analyze input data' });
space.addKnowledgeDomain('metrics', { threshold: 0.9, unit: 'percent' });
inspect();
```
