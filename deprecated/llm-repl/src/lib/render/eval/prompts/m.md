...
**display(ui, opts?)**: non-blocking. Renders `ui` in the output panel. Stable `id` updates element in place (replace mode default when id is set). Fire-and-forget — do not await. `maxEntries` budget applies; oldest entries dropped first.

**ask<T>(ui, opts?)**: returns `Promise<T>`. Non-blocking. 5-minute timeout independent of inspect() soft cap. On timeout: resolves with `fallback` if provided, else rejects with TimeoutError. Use to collect user input at the right moment — don't block execution unnecessarily.
