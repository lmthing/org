...
**display(ui, opts?)**: non-blocking. Renders `ui` in the output panel. Stable `id` updates element in place (replace mode default when id is set). Fire-and-forget — do not await. `maxEntries` budget applies; oldest non-stable entries dropped first when exceeded.

**ask<T>(ui, opts?)**: returns `Promise<T>`. Non-blocking. 5-minute logical timeout independent of inspect() soft cap. On timeout: resolves with `fallback` if provided, else rejects with `TimeoutError`. Execution continues while waiting — the promise resolves when the host renderer calls submit. Multiple asks can be in-flight concurrently.

**Built-in components**: TextInput, Select, Confirm, Table, ProgressBar, Markdown, CodeBlock. Use JSX syntax: `<TextInput label="Name?" />`. Components are available as globals.

**On rollback**: display entries added after the rollback target are invalidated. Pending asks remain active.

## When to use display() vs ask()

- Use `display()` for output, status, progress, tables, code — anything that doesn't need user input.
- Use `ask()` when you need a value from the user before proceeding. Always handle the fallback case.
- Prefer stable `id` when updating a widget repeatedly (progress bars, status messages).
- Chain `display()` before `ask()` to show context to the user before asking.
- Never use console.log for user-facing output — use display() instead.
- Multiple `ask()` calls can be in-flight concurrently — they each resolve independently when submitted.
- Components can be composed: `<Table data={rows} />`, `<ProgressBar value={pct} label="Processing..." />`.

## Eval Instructions

Write a TypeScript completion for the task. Use display() for output and ask() for user input appropriately. Apply stable ids where repeated updates make sense. End with inspect().
