...
**display(ui, opts?)**: non-blocking. Renders `ui` in the output panel. Stable `id` updates element in place (replace). Fire-and-forget — no await.

**ask<T>(ui, opts?)**: returns `Promise<T>`. Non-blocking. 5-min timeout. Use to collect user input. Pass result to inspect() to see current state.

## Eval Instructions

Complete the given TypeScript task using display() and ask() as needed. End with inspect().
