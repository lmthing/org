...
| **P2 — BEM → styled()+variants** | 🟡 all 15 leaves landed | Every leaf `elements/**` BEM block → a `*.styled.tsx` `styled()`+variants, each with a `*-styled.test.tsx` gate (structure + tokens + jsdom render, all green): forms (button/input/select/textarea), content (avatar/badge/card/list-item/panel/separator), typography (caption/code/heading/label), branding (cozy-text). **15/68** CSS files. Remaining 53 = the composite `components/**` (39) + the composite/nav/layout `elements/**` (nav/layouts/terminal) + the overlays (dialog/dropdown/sheet, on the P4 track): per-slice |
