...
| **P2 — BEM → styled()+variants** | 🟡 leaves landing | `.btn` → `libs/ui/src/elements/forms/button/button.styled.tsx` (the §4 example) + `.input` → `libs/ui/src/elements/forms/input/input.styled.tsx` (2nd leaf), each with a variant-structure test green. Remaining 66 CSS files: per-slice |
