...
| **P0 — real-surface visual harness** | ✅ **BUILT — baseline captured** | `apps/web/tests/visual-surface/`: the SHIPPED components from `@lmthing/ui`, in the app's own `TamaguiProvider`, under the real `theme.css` (Tailwind preflight + tokens) AND `chat/app/styles.css` (the `lm-*` keyframes). 8 fixtures, **212 elements across light + dark**, ~70 audited computed properties each, committed to `__baseline__/`. `pnpm test:surface` compares and exits 1 on any delta; `pnpm test:surface:update` re-captures. Two fixtures are there for specific reasons: `state-props` (hover/press/focus + the `group`/`$group-*-hover` pair, both halves of which fail silently if they come apart) and `animation` (the 67 remaining `transition-*`/`lm-*` classNames, with their real resolved values, so the driver swap is a readable delta). Animations are frozen at t=0 during the walk, or `lm-fade-in`/`lm-pulse` make the gate flap. This does NOT supersede `tests/visual/` — that harness renders local PASSTHROUGH copies of the pre-Tamagui primitives to keep its pre-swap baselines valid, so it says nothing about what ships |
